-- ============================================================================
-- Admin usage dashboard — program-wide engagement aggregates for is_admin() only.
-- Raw answer rows stay private (RLS); this SECURITY DEFINER RPC returns only
-- rollups + first-name-level leaderboards for residency leadership.
-- ============================================================================
...
as $$
declare
  d int := greatest(7, least(coalesce(days_back, 90), 365));
  since timestamptz := (current_date - d)::timestamptz;
...
begin
  ...
  with
  -- Approved humans (exclude test / placeholder accounts)
  members as (
    select p.*
    from profiles p
  ...
  daily as (
    select
      ds.day::text as day,
      coalesce(p.practice_answers, 0) as practice_answers,
      coalesce(o.poll_answers, 0) as poll_answers,
      coalesce(p.practice_answers, 0) + coalesce(o.poll_answers, 0) as total_answers,
      -- unique people active that day (practice ∪ poll)
      (
      ...
      case when coalesce(p.practice_answers, 0) > 0
        then round(100.0 * p.first_ok / p.practice_answers)
        else null
      end as first_try_pct
    from day_spine ds
    left join practice_by_day p on p.day = ds.day
    left join poll_by_day o on o.day = ds.day
    order by ds.day
  ),
  roster as (
    select
  ...
  select jsonb_build_object(
    ...
    'polls', (select to_jsonb(p) from poll_sum p),
...
comment on function admin_usage_dashboard(int) is
  'Admin-only residency usage dashboard aggregates (practice + polls over time).';
