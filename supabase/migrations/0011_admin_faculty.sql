-- ============================================================================
-- Grant automatic admin role to Dr. Nelson, Dr. Hester, and Dr. Baker.
--
-- Strategy: set class_year='admin' in roster_names for these three so that
-- handle_new_user() (updated below) assigns role='admin' on first sign-in.
-- Also update any profiles that already exist with matching names.
-- ============================================================================

-- 1. Mark the three as 'admin' in the name roster
update roster_names set class_year = 'admin'
where (first_name = 'Suzie'    and last_name = 'Nelson')
   or (first_name = 'Jonathan' and last_name = 'Hester')
   or (first_name = 'Matthew'  and last_name = 'Baker');

-- 2. Update existing profiles that already signed in under these names
update profiles set role = 'admin', status = 'approved'
where lower(full_name) like 'suzie%nelson'
   or lower(full_name) like 'jonathan%hester'
   or lower(full_name) like 'matthew%baker';

-- 3. Update handle_new_user() to assign role='admin' when class_year='admin'
