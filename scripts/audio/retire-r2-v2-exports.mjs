...
const DELETE_STATE = resolve(ROOT, "extraction/output/audio_r2_v2_delete.state.json");
...
async function main() {
  ...
  if (manifest.version !== "v4-r2-48k-complete") throw new Error("Production manifest is not the complete v3 archive");
  if (!newState.completed_at || Object.keys(newState.uploaded ?? {}).length !== 392) throw new Error("The 392-object v3 verification gate is incomplete");
  const rows = Object.entries(oldState.uploaded ?? {})
    .filter(([key]) => key.startsWith(PREFIX))
    .map(([key, value]) => ({ key, bytes: Number(value.bytes) }))
    .sort((a, b) => a.key.localeCompare(b.key));
  if (rows.length !== 226 || rows.some((row) => !/^exports\/v2\/[a-z0-9-]+\.mp3$/.test(row.key) || !Number.isInteger(row.bytes) || row.bytes < 500)) {
    throw new Error(`Refusing unexpected v2 inventory (${rows.length} keys)`);
  }
  const bytes = rows.reduce((sum, row) => sum + row.bytes, 0);
  console.log(`[guard] deleting exactly ${rows.length} ${PREFIX} objects (${(bytes / 1024 ** 3).toFixed(3)} GiB); source clips and v3 excluded`);
  let deleted = 0;
  ...
  console.log(`[done] retired ${rows.length} v2 R2 objects; ${(bytes / 1024 ** 3).toFixed(3)} GiB removed`);
}
