...
type Blob = {
  ...
  /** What comes first: restrict today's mix to unseen questions (default off). */
  unseen_only?: boolean;
  owl_on?: boolean;
  fox_on?: boolean;
};
...
function snapshot(uid: string): Blob {
  return {
    ...
    unseen_only: readJson<boolean>("pd_unseen_only", false) === true,
...
export function syncClientPrefs(uid: string, remoteRaw: Record<string, unknown> | null | undefined) {
  ...
  const merged: Blob = {
    ...
    unseen_only: typeof r.unseen_only === "boolean" ? r.unseen_only : l.unseen_only === true,
  ...
  writeJson("pd_unseen_only", merged.unseen_only === true);
