...
/** A study queue should include only one cross-year copy of a repeated PRITE
 * item. Input order decides the representative, so callers sort first. */
...
/** Deterministically mixes equally high-yield groups without using a random
 * comparator, which would make Array.sort unstable. */
...
function orderComparator(opts: {
  ...
  const score = (q: RawQuestion, rule: OrderRuleId): number => {
    switch (rule) {
      ...
      case "highyield": {
        // Preserve repeat-count tiers, mix concepts within a tier, and put a
        // concept answered this week behind equally repeated new concepts.
...
export default function App() {
  ...
  const startReview = useCallback(() => {
    ...
    setReviewMode(true);
    setTodayQueue(uniqueQuestionGroups(missed).slice(0, 30));
